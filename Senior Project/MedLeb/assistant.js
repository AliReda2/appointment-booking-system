...
document.addEventListener('DOMContentLoaded', function () {
	...
	if (changeFormElement) {
		changeFormElement.addEventListener('submit', function (event) {
			...
			xhr.open('POST', 'update-assistant.php', true);
			...
			xhr.send(`newUsername=${encodeURIComponent(newUsername)}&newPassword=${encodeURIComponent(newPassword)}&newPhone=${encodeURIComponent(newPhone)}&newEmail=${encodeURIComponent(newEmail)}`);
		});
	}
});
