...
if (empty($newUsername) || empty($newPassword) || empty($newPhone) || empty($newEmail)) {
...
$sql = "UPDATE assistant SET name = ?, password = ?, phone = ?, email = ? WHERE id = ?";
...
$assistantId = $Aid;
$stmt->bind_param("sssss", $newUsername, $hashedPassword, $newPhone, $newEmail, $assistantId);
...
error_log("Parameters: username={$newUsername}, password={$hashedPassword}, phone={$newPhone}, email={$newEmail}, id={$assistantId}");
